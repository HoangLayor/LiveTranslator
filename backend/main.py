...
from backend.database import engine
from backend.models.feedback_model import Base
from backend.api import image_routers
...
app.include_router(file_routes.router, prefix="/api", tags=["file-translator"])
app.include_router(feedback_routes.router, prefix="/api", tags=["feedback"])
app.include_router(image_routers.router, prefix="/api", tags=["image-translator"])
                ...
                if "text" in data:
                    try:
                        ...
                        if message.get("type") == "translate":
                            ...
                            logger.info(
                                f"Translate from {source_lang} to {target_lang}: {text} -> {translated_text}"
                            )
                            await websocket.send_json(
                                {
                                    "type": "translation",
                            ...
                            await websocket.send_json(
                                {"type": "audio", "audio": audio, "text": text}
                            )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Speech to Text with {lang}: {audio_data} -> {text}"
                        )
